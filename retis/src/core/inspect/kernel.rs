...
impl KernelInspector {
    ...
    pub(crate) fn from(kconf: Option<&PathBuf>) -> Result<KernelInspector> {
        ...
        let inspector = KernelInspector {
            ...
            traceable_events: Self::file_to_hashset(events_file), // /sys/kernel/debug/tracing/available_events
            // Not all functions we'll get from BTF/kallsyms are traceable. Use
            // the following, when available, to narrow down our checks.
            traceable_funcs: Self::file_to_hashset(funcs_file), // /sys/kernel/debug/tracing/available_filter_functions
            version,
            config,
            modules: Self::file_to_hashset(modules_file), // /proc/modules
        };
